...
from backend.models.webstoremodels import WomensProducts
    ...
    def get(self):
        ...
        if token and token.startswith('Bearer '):
            ...
            if user.user_type == 'admin' or user.user_type == 'customer' or user.user_type == 'employee':
                products = WomensProducts.query.filter_by(category_id=1).all()
                if products:
                    product_list = [product.to_dict() for product in products]  # Convert each object
                    return jsonify(product_list)
                else:
                    return jsonify({"message": "No products available currently"}) #, "code": 501
...
class Womensclothes(restful.Resource):
    def delete(self, product_id):
        ...
        if token and token.startswith('Bearer '):
            ...
            product = WomensProducts.query.get(product_id)
            if not product:
